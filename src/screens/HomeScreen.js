...
import { View, Text, FlatList, TouchableOpacity, TextInput, StyleSheet, Image, ScrollView, Alert,ActivityIndicator, useWindowDimensions } from 'react-native';
import Ionicons from 'react-native-vector-icons/Ionicons';
import BookCard from '../components/Book';
import { createTable } from '../databases/dbHelper';
import { saveBooks, checkBooksInDatabase } from '../databases/dbHelper';
...
const HomeScreen = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
        ...
        console.error('Error fetching data, loading from SQLite:', error);
        getBooks((books) => {
            setData(books);
  ...
  if (isLoading) {
    return <ActivityIndicator size="large" color="#33a67b" style={{ flex: 1 }} />;
  }
